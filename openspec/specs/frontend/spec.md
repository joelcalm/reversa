...
The React/Vite/TypeScript web app: an institutional dashboard, the four briefing pages, and a general
graph explorer, all rendering Cytoscape subgraphs sourced from the backend.

## Requirements

### Requirement: Dashboard
The frontend SHALL provide a landing dashboard titled "BOE Knowledge Graph" that displays summary KPI
cards, four briefing entry cards, a clearly stated default scope note, and data freshness/ingestion
stats.
...
The frontend SHALL provide a dedicated page for each of the four briefings, each with a plain-language
explanation, the relevant table/KPIs, and a graph view sourced from the backend graph endpoints.
...
The frontend SHALL provide a general graph explorer where users can search a norm by text or BOE id,
open its neighborhood, filter by relation type, and click a node to view a side panel of details.
...
Cytoscape graphs SHALL style nodes by lifecycle status (live vs repealed/annulled/expired) and edges
by normalized relation type, using CSS variables and an institutional white/green theme, and all data
views SHALL handle loading, error, and empty states.
